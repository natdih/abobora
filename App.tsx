...
function isUnauthorizedError(error: unknown) {
  return error instanceof Error && /login|autentic|401|senha/i.test(error.message);
}
...
function App() {
  ...
  useEffect(() => {
    const storedToken = api.restoreToken();
  ...
  useEffect(() => {
    ...
    refreshBets(competitionId).catch((error) => {
      if (isUnauthorizedError(error)) {
        logout();
        return;
      }
      ...
      <main className="min-h-screen px-3 py-4 sm:px-5 lg:px-8">
        ...
        <div className="mx-auto flex min-h-[80vh] max-w-xl items-center justify-center p-4">
          <Card className="w-full">
            <CardHeader>
              <CardTitle>Preparando o acesso</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground">Verificando o login e carregando os dados...</p>
            </CardContent>
          </Card>
        </div>
      </main>
            ...
            <CardContent>
              <form className="grid gap-3" onSubmit={handleLogin}>
                <p className="text-sm text-muted-foreground">
                  Digite a senha para acessar as apostas e editar os dados.
                </p>
