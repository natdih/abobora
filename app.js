...
const adminPassword = process.env.APP_ADMIN_PASSWORD || process.env.GOOGLE_SHEETS_WEBHOOK_SECRET || "abobora-2026";
...
function requireAuth(req, res, next) {
  ...
  res.status(401).json({ message: "Acesso restrito. Faça login para continuar." });
}
...
function validateBet(body) {
  ...
  if (phoneDigits && (phoneDigits.length < 10 || phoneDigits.length > 11)) return { error: "Informe um telefone brasileiro valido." };
...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, database: "sqlite", path: dbPath });
});

app.get("/api/competitions", requireAuth, (_req, res) => {
...
app.get("/api/competitions/:competitionId/bets", requireAuth, (req, res) => {
...
app.post("/api/competitions/:competitionId/bets", requireAuth, (req, res) => {
  const competitionId = Number(req.params.competitionId);
...
app.post("/api/competitions/:competitionId/bets/import", requireAuth, (req, res) => {
  const competitionId = Number(req.params.competitionId);
...
app.put("/api/bets/:id", requireAuth, (req, res) => {
